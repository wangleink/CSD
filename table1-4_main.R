...
for (r in 1:Run){
  ...
  beta_ave[((r-1)*d+1):(r*d),1]=apply(beta_hat1,1,mean)
  ...
  ####################################################################################sub方法
  beta_sub[((r-1)*d+1):(r*d),1]=beta_hat1[,1]
  beta_sub_pqr[((r-1)*d+1):(r*d),1]=beta_hat8[,1]
  ####################################################################################full方法
  beta_full[((r-1)*d+1):(r*d),1]=rq.fit.lasso(cbind(1,x,z),y,tau=Tau,lambda = 1)$coefficient[2:(d+1)]
...
#####################################################################################################csl_all方法
  ...
  beta00[((r-1)*d+1):(r*d),1]=beta_all[,1]
  beta11[((r-1)*d+1):(r*d),1]=beta_hat
  beta22[((r-1)*d+1):(r*d),1]=result6$coefficient[1:d]
  print(r)
}
...
msesub_all=rep(0,Run)
madsub_all=rep(0,Run)
msesubpqr_all=rep(0,Run)
madsubpqr_all=rep(0,Run)

mseave_all=rep(0,Run)
madave_all=rep(0,Run)
mseavepqr_all=rep(0,Run)
madavepqr_all=rep(0,Run)

msefull_all=rep(0,Run)
madfull_all=rep(0,Run)
msefullpqr_all=rep(0,Run)
madfullpqr_all=rep(0,Run)
...
for (r in 1:Run){
  mse00[r]=sum( (beta00[((r-1)*d+1):(r*d),1]-beta0)^2 )
  mse11[r]=sum( (beta11[((r-1)*d+1):(r*d),1]-beta0)^2 )
  mse22[r]=sum( (beta22[((r-1)*d+1):(r*d),1]-beta0)^2 )
  ...
  msesub_all[r]=sum( (beta_sub[((r-1)*d+1):(r*d),1]-beta0)^2 )
  madsub_all[r]=sum( abs(beta_sub[((r-1)*d+1):(r*d),1]-beta0) )
  msesubpqr_all[r]=sum( (beta_sub_pqr[((r-1)*d+1):(r*d),1]-beta0)^2 )
  madsubpqr_all[r]=sum( abs(beta_sub_pqr[((r-1)*d+1):(r*d),1]-beta0) )
  
  mseave_all[r]=sum( (beta_ave[((r-1)*d+1):(r*d),1]-beta0)^2 )
  madave_all[r]=sum( abs(beta_ave[((r-1)*d+1):(r*d),1]-beta0) )
  mseavepqr_all[r]=sum( (beta_ave_pqr[((r-1)*d+1):(r*d),1]-beta0)^2 )
  madavepqr_all[r]=sum( abs(beta_ave_pqr[((r-1)*d+1):(r*d),1]-beta0) )
  
  msefull_all[r]=sum( (beta_full[((r-1)*d+1):(r*d),1]-beta0)^2 )
  madfull_all[r]=sum( abs(beta_full[((r-1)*d+1):(r*d),1]-beta0) )
  msefullpqr_all[r]=sum( (beta_full_pqr[((r-1)*d+1):(r*d),1]-beta0)^2 )
  madfullpqr_all[r]=sum( abs(beta_full_pqr[((r-1)*d+1):(r*d),1]-beta0) )
...
msesub=mean(msesub_all)
madsub=mean(madsub_all)
msesubpqr=mean(msesubpqr_all)
madsubpqr=mean(madsubpqr_all)

mseave=mean(mseave_all)
madave=mean(madave_all)
mseavepqr=mean(mseavepqr_all)
madavepqr=mean(madavepqr_all)

msefull=mean(msefull_all)
madfull=mean(madfull_all)
msefullpqr=mean(msefullpqr_all)
madfullpqr=mean(madfullpqr_all)
...
mean(mse22)
rname = c("mse","mad")
cname = c("sub","ave","full","subpqr","avepqr","fullpqr","pqr1","pqr2","pqrave1","pqrave2")
